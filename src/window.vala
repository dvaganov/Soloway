...
public class SWindow : Gtk.ApplicationWindow
{
    private Gtk.ListBox _playlist;
    private SHorizontalPanel _panel;
    private SSidePanel _side_panel;
    private Gtk.HeaderBar _header_bar;
    
    public signal void on_row_activate (string uri);
    
    public SWindow (int width, int height)
    {
        show_menubar = false;
        default_width = width;
        ...
        _panel = new SHorizontalPanel ();
        main_grid.attach (_panel, 0, 1, 2, 1);
        
        _side_panel = new SSidePanel ();
    ...
    private void _set_side_panel_btn ()
    {
        ...
        {
            var entry = _playlist.get_selected_row () as SEntry;
    ...
    {
        var entry = new SEntry (title, uri);
        _playlist.add (entry);
    }
    ...
    public void change_btn_state_to_play (bool is_playing)
    {
        ...
        if (is_playing) _panel.reveal_child = true;
        else _panel.reveal_child = false;
    }
    public void activate_next_row ()
    {
        ...
        if (next_row != null) next_row.activate ();
    }
    public void activate_prev_row ()
    {
        ...
        if (prev_row != null) prev_row.activate ();
    }
